...
2.  Write an extension in C or a language that can output C symbols. More info in [The API](#the-abi).
3.  Compile your extension as a dynamic library and move to the extensions directory. (You can disable it with -- prefix).

## The API
All API functions take in a `const photon_api_t *` parameter. To get access to types include `src/photon.h` in your code.

Here are all the API functions:
